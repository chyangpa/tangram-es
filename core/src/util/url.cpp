...
size_t removeLastSegmentFromRange(std::string& string, size_t start, size_t end) {
...
Url Url::resolve(const Url& b, const Url& r) {
    ...
    if (r.hasParameters()) {
        ...
        // Otherwise use the base parameters?
        t.parts.parameters.count = 0;
    }
...
size_t Url::removeDotSegmentsFromRange(std::string& string, size_t start, size_t count) {

    // Implements https://tools.ietf.org/html/rfc3986#section-5.2.4
    // with in-place manipulation instead of making a new buffer.

    char* begin = &string.front() + start;
    char* end = &string.front() + start + count;
